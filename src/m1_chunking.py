...
def chunk_hierarchical(text: str, parent_size: int = HIERARCHICAL_PARENT_SIZE,
    ...
    for para in paragraphs:
        if len(current_parent_text) + (2 if current_parent_text else 0) + len(para) > parent_size:
    ...
    for p in parents:
        ...
        # Split parent text into sentences/paragraphs
        sentences = re.split(r'(?<=[.!?])\s+|\n\n', p.text)
        sentences = [s.strip() for s in sentences if s.strip()]
        
        current_child_text = ""
        for s in sentences:
            if len(current_child_text) + (1 if current_child_text else 0) + len(s) > child_size:
